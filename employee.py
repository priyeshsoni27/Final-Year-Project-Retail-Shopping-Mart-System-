...
class bill_window:
    def __init__(self, top=None):
        ...
        self.img = PhotoImage(file="./images/bill_window.jpeg")
        ...
        self.button6.configure(command=self.ptr)
    ...
    def ptr(self):
        #print([cust_search_bill.get().rstrip()])
        call="SELECT * FROM bill WHERE bill_no = ?"
        cur.execute(call, [cust_search_bill.get().rstrip()])
        results = cur.fetchall()
        #print(results)
        billno=results[0][0]
        date=results[0][1]
        ...
        #myTable = PrettyTable(["Bill Number","Name", "Num","Date", "Amount"])
 

        #myTable.add_row([name, num, data[0][-1]])
        ...
        ok=st[2]
        #print(ok)
        #final=[int(s)for s in st.split() if s.isdigit()]
        #print(final)
        #final_ans=final[0]*final[1]

        header = ["Bill Number","Name", "Num","Date", "Amount"]
        data = [billno,name, num,date, ok]


        with open('C:/Users/Rahul badlani/ok.csv', 'w', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
